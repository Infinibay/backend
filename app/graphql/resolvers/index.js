...
const resolvers = {
  Query: {
    ...
    getUserList: async (parent, input) => {
      try {
        ...
        if (decoded.userType == "admin") {
          ...
          if (search && decoded.userType == "admin") {
    ...
    getUserByID: async (parent, input) => {
      try {
        ...
        if (decoded_id && decoded.userType == "admin") {
    ...
    getSpecificVM: async (parent, input) => {
      try {
        ...
        input.userId = decoded;
        const decoded_id = input.userId.id;
        // const for_type = input.userId.User_Type
        console.log(decoded.userType);

        if (decoded_id && decoded.userType == "user") {
    ...
    getNotification: async () => {
      try {
        const for_getNotification = await prisma.notification.findMany({});
        return for_getNotification;
      } catch (error) {
        return error;
      }
    },
  ...
  Mutation: {
    ...
    async createUser(root, input) {
      try {
        //for Image
        const path = "app/User_Image/" + Date.now() + ".jpeg";
        const User_Image = input["input"]["userImage"];
        if (User_Image) {
          var base64Data = await User_Image.replace(
            /^data:([A-Za-z-+/]+);base64,/,
            ""
          );
        ...
        if (decoded_id) {
          const for_Update_User = await prisma.user.update({
            ...
            data: {
              ...
              Deleted: input["input"]["Deleted"],
              // User_Image: input["input"]["User_Image"] ,
              User_Image: path,
    ...
    async deleteVM(root, input) {
      try {
        ...
        if (decoded_id) {
          ...
          const for_deleteVM = await prisma.virtualMachine.findMany({
            where: {
              //  user: decoded_id,
              id: { in: id },
              userId: decoded_id,
          ...
          console.log(for_deleteVM);
          console.log(for_deleteVM.length);
          //}
          // console.log(decoded_id);
          for (var i = for_deleteVM.length; i >= 0; i++) {
            console.log({ in: id });
            console.log(i);
            if (
              for_deleteVM
              ...
              console.log(for_not);
              const deleteVM_id = await prisma.virtualMachine.deleteMany({
                where: {
                  id: { in: id },
                  userId: decoded_id,
                },
              });
              console.log(deleteVM_id);
              if (deleteVM_id) {
                console.log("del");
              }
              return "VM_Deleted";
    ...
    async deleteNotification(root, input) {
      try {
        const for_deleteNotification = await prisma.notification.delete({
          where: {
            id: input["input"]["id"],
          },
        });
    ...
    async createISO(root, input) {
      try {
        const for_createISO = await prisma.IOS.create({
          data: {
    ...
    //FOR DELETE IOS
    async deleteISO(root, input) {
      try {
        const for_deleteISO = await prisma.IOS.delete({
          where: {
            id: input["input"]["id"],
          },
        });
        console.log(for_deleteISO);
        return "IOS Deleted";
      } catch (error) {
        console.log(error);
        return error;
