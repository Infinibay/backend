import { CronExpression, Scheduler, createScheduleAdapter, describeCron } from '@main/lib/scheduler'

describe('CronExpression', () => {
  describe('parsing', () => {
    it('accepts standard 5-field expressions', () => {
      expect(() => new CronExpression('0 0 * * *')).not.toThrow()
      expect(() => new CronExpression('*/5 * * * *')).not.toThrow()
      expect(() => new CronExpression('0 9-17 * * 1-5')).not.toThrow()
      expect(() => new CronExpression('0,15,30,45 * * * *')).not.toThrow()
    })

    it('rejects invalid expressions', () => {
      expect(() => new CronExpression('* * * *')).toThrow(/expected 5 fields/)
      expect(() => new CronExpression('60 * * * *')).toThrow(/out of bounds/)
      expect(() => new CronExpression('* 24 * * *')).toThrow(/out of bounds/)
      expect(() => new CronExpression('foo * * * *')).toThrow()
    ...
    it('treats day-of-week 7 as Sunday', () => {
      ...
      const expr = new CronExpression('0 12 * * 7')
  ...
  describe('matches', () => {
    it('matches exact times', () => {
      const expr = new CronExpression('30 14 * * *')
    ...
    it('honors step values', () => {
      const expr = new CronExpression('*/15 * * * *')
    ...
    it('ORs dom and dow when both are restricted', () => {
      const expr = new CronExpression('0 0 1 * 0')
  ...
  describe('next', () => {
    it('finds the next match', () => {
      const expr = new CronExpression('0 3 * * *')
    ...
    it('advances to next month when needed', () => {
      const expr = new CronExpression('0 0 15 * *')
...
describe('Scheduler', () => {
  ...
  beforeEach(() => { scheduler = new Scheduler({ tickMs: 60_000 }) })
...
describe('createScheduleAdapter', () => {
  it('returns a ScheduledJob compatible with infinization', () => {
    ...
    const adapter = createScheduleAdapter(scheduler)
    const fn = jest.fn()
...
describe('describeCron', () => {
  ...
  it('falls back for non-trivial expressions', () => {
    expect(describeCron('*/15 * * * *')).toContain('Custom')
  })
