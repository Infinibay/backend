...
const mockVirtioSocketWatcher = (0, jest_mock_extended_1.mockDeep)();
describe('ProcessManager', () => {
    let processManager;
    beforeEach(() => {
        jest.clearAllMocks();
        processManager = new ProcessManager_1.ProcessManager(mockPrisma, mockVirtioSocketWatcher);
    ...
    describe('listProcesses', () => {
        it('should list processes via VirtIO socket when available', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            mockVirtioSocketWatcher.sendSafeCommand.mockResolvedValue({
                success: true,
                data: mockProcesses
            });
            const result = yield processManager.listProcesses(machineId);
            ...
            expect(mockVirtioSocketWatcher.sendSafeCommand).toHaveBeenCalledWith(machineId, { action: 'ProcessList', params: undefined }, 30000);
        }));
        it('should throw error when VirtIO is not available', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            yield expect(processManager.listProcesses(machineId))
                .rejects.toThrow('Failed to get process list: InfiniService not available');
        }));
        it('should throw error when machine is not running', () => __awaiter(void 0, void 0, void 0, function* () {
            const machineId = 'stopped-vm';
            // Mock to simulate that machine is not running (getRunningMachine returns null)
            const mockProcessManager = new ProcessManager_1.ProcessManager(mockPrisma, mockVirtioSocketWatcher);
            const mockGetRunningMachine = jest.spyOn(mockProcessManager, 'getRunningMachine');
            mockGetRunningMachine.mockResolvedValue(null);
            yield expect(mockProcessManager.listProcesses(machineId))
                .rejects.toThrow(`Machine ${machineId} is not available`);
        }));
    });
    describe('killProcess', () => {
        it('should kill a process successfully via VirtIO', () => __awaiter(void 0, void 0, void 0, function* () {
            const machineId = 'test-vm-1';
            const pid = 1234;
            const mockMachine = {
                id: machineId,
            ...
            const result = yield processManager.killProcess(machineId, pid, false);
            expect(result).toEqual({
                success: true,
                message: `Process ${pid} terminated successfully`,
                pid
            });
            expect(mockVirtioSocketWatcher.sendSafeCommand).toHaveBeenCalledWith(machineId, { action: 'ProcessKill', params: { pid, force: false } }, 30000);
        }));
        it('should handle kill failure gracefully', () => __awaiter(void 0, void 0, void 0, function* () {
            const machineId = 'test-vm-1';
            const pid = 9999;
            const mockMachine = {
                id: machineId,
        ...
        it('should return error when VirtIO is not available for kill', () => __awaiter(void 0, void 0, void 0, function* () {
            const machineId = 'test-vm-1';
            const pid = 1234;
            const mockMachine = {
                id: machineId,
    ...
    describe('getTopProcesses', () => {
        it('should return top processes sorted by CPU', () => __awaiter(void 0, void 0, void 0, function* () {
            const machineId = 'test-vm-1';
            const mockMachine = {
                id: machineId,
                name: 'test-vm',
            ...
            const result = yield processManager.getTopProcesses(machineId, 2);
            expect(result).toHaveLength(2);
            expect(result[0].name).toBe('high-cpu');
            expect(result[1].name).toBe('med-cpu');
            // Verify the command was sent with correct params
            expect(mockVirtioSocketWatcher.sendSafeCommand).toHaveBeenCalledWith(machineId, { action: 'ProcessTop', params: { limit: 2, sort_by: 'cpu' } }, 30000);
        }));
        it('should return top processes sorted by memory', () => __awaiter(void 0, void 0, void 0, function* () {
            const machineId = 'test-vm-1';
            const mockMachine = {
                id: machineId,
                name: 'test-vm',
            ...
            const result = yield processManager.getTopProcesses(machineId, 2, ProcessSortBy.MEMORY);
            expect(result).toHaveLength(2);
            expect(result[0].name).toBe('high-mem');
            expect(result[1].name).toBe('med-mem');
            // Verify the command was sent with correct params
            expect(mockVirtioSocketWatcher.sendSafeCommand).toHaveBeenCalledWith(machineId, { action: 'ProcessTop', params: { limit: 2, sort_by: 'memory' } }, 30000);
        }));
    });
    describe('killProcesses', () => {
        it('should kill multiple processes', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            mockPrisma.machine.findUnique.mockResolvedValue(mockMachine);
            mockVirtioSocketWatcher.sendSafeCommand
                .mockResolvedValueOnce({ success: true })
                .mockResolvedValueOnce({ success: true });
            const results = yield processManager.killProcesses(machineId, pids, false);
            expect(results).toHaveLength(2);
            expect(results[0].success).toBe(true);
            expect(results[1].success).toBe(true);
            expect(mockVirtioSocketWatcher.sendSafeCommand).toHaveBeenCalledTimes(2);
        }));
        it('should handle partial failures when killing multiple processes', () => __awaiter(void 0, void 0, void 0, function* () {
            const machineId = 'test-vm-1';
