...
import { CommandDispatcher, CommandDispatcherDeps } from '../../../../app/services/socket-watcher/CommandDispatcher'
import type { VmConnection, CommandResponse, SafeCommandType } from '../../../../app/services/socket-watcher/types'
import { createLogger } from 'winston'

// ─── Helpers ────────────────────────────────────────────────────────────────

function makeConnection(overrides?: Partial<VmConnection>): VmConnection {
  return {
    vmId: 'vm-1',
    socket: {} as any,
    ...
    healthCheckResults: [],
    messageStats: { sent: 0, received: 0, errors: 0, totalBytes: 0, averageLatency: 0 },
    ...
    circuitBreakerLastStateChange: new Date(),
    keepAliveSequence: 0,
...
function makeDeps(overrides?: Partial<CommandDispatcherDeps>): CommandDispatcherDeps {
  const connections = new Map<string, VmConnection>()
  const connection = makeConnection()
  connections.set('vm-1', connection)

  return {
    debug: createLogger({ silent: true }),
    connections,
    reconnectFn: jest.fn().mockResolvedValue(undefined),
    sendMessage: jest.fn(),
    ...overrides,
  }
}
...
function resolveLastPendingCommand(
...
describe('CommandDispatcher', () => {
  ...
  let deps: CommandDispatcherDeps

  beforeEach(() => {
    jest.clearAllMocks()
    deps = makeDeps()
    dispatcher = new CommandDispatcher(deps)
  })

  // ─── sendSafeCommand ────────────────────────────────────────────────────

  describe('sendSafeCommand', () => {
    it('sends a safe command and resolves with response', async () => {
      const response = makeSuccessResponse()
      const commandType: SafeCommandType = { action: 'SystemInfo' }
      ...
      const [conn, message] = (deps.sendMessage as jest.Mock).mock.calls[0]
      expect(conn.vmId).toBe('vm-1')
      expect(message.type).toBe('SafeCommand')
      expect(message.command_type).toEqual({ action: 'SystemInfo' })
    })

    it('throws if no connection exists for VM', async () => {
      const commandType: SafeCommandType = { action: 'SystemInfo' }

      await expect(
        dispatcher.sendSafeCommand('vm-unknown', commandType),
      ).rejects.toThrow('No connection to VM vm-unknown')
    })

    it('throws if VM is disconnected and socket file not found', async () => {
      ...
      await expect(
        dispatcher.sendSafeCommand('vm-1', commandType),
      ).rejects.toThrow('not connected and socket file not found')
    })
  ...
  describe('sendUnsafeCommand', () => {
    it('sends an unsafe command and resolves with response', async () => {
      const response = makeSuccessResponse({ command_type: 'unsafe' })

      const sendPromise = dispatcher.sendUnsafeCommand(
        'vm-1',
        'echo hello',
        { shell: 'bash', runAs: 'root' },
        10000,
      )
      ...
      const [, message] = (deps.sendMessage as jest.Mock).mock.calls[0]
    ...
    it('throws if no connection exists for VM', async () => {
      await expect(
        dispatcher.sendUnsafeCommand('vm-unknown', 'ls'),
      ).rejects.toThrow('No connection to VM vm-unknown')
    })
    ...
    it('throws if VM is disconnected and socket file not found', async () => {
      ...
      await expect(
        dispatcher.sendUnsafeCommand('vm-1', 'ls'),
      ).rejects.toThrow('not connected and socket file not found')
    })
  })
  ...
  describe('executeCommandWithRetry', () => {
    it('returns response on first attempt when successful', async () => {
      const response = makeSuccessResponse()
      const builder = jest.fn().mockResolvedValue(response)

      const result = await dispatcher.executeCommandWithRetry('vm-1', builder, 3, 10)
    ...
    it('throws after all retries exhausted', async () => {
      ...
      await expect(
        dispatcher.executeCommandWithRetry('vm-1', builder, 2, 10),
      ).rejects.toThrow('Persistent failure')

      expect(builder).toHaveBeenCalledTimes(2)
    })

    it('returns failed response on last attempt even if not success', async () => {
      const failedResponse: CommandResponse = {
        success: false,
        exit_code: 1,
        error: 'Something went wrong',
      }
      const builder = jest.fn()
        .mockResolvedValueOnce({ ...failedResponse, error: 'first fail' })
        .mockResolvedValueOnce(failedResponse)

      const result = await dispatcher.executeCommandWithRetry('vm-1', builder, 2, 10)
  ...
  describe('formatCommandType', () => {
    it('formats PackageSearch with query param', async () => {
      const response = makeSuccessResponse()
    ...
    it('formats ExecutePowerShellScript with required params', async () => {
      const response = makeSuccessResponse()
      const commandType: SafeCommandType = {
    ...
    it('throws for ExecutePowerShellScript without script param', async () => {
      const commandType: SafeCommandType = {
        action: 'ExecutePowerShellScript',
        params: { script: '' },
      }
      ...
      await expect(
        dispatcher.sendSafeCommand('vm-1', commandType, 60000),
      ).rejects.toThrow('non-empty script parameter')
    })

    it('formats ProcessKill with pid and force', async () => {
      const response = makeSuccessResponse()
      const commandType: SafeCommandType = {
        action: 'ProcessKill',
    ...
    it('formats RunMaintenanceTask with all params', async () => {
      const response = makeSuccessResponse()
      const commandType: SafeCommandType = {
    ...
    it('formats UserList action', async () => {
      const response = makeSuccessResponse()
      const commandType: SafeCommandType = {
        action: 'UserList',
      }
  ...
  describe('convenience wrappers', () => {
    async function testWrapper(fn: () => Promise<CommandResponse>) {
      const response = makeSuccessResponse()
      const promise = fn()
      await new Promise(resolve => setTimeout(resolve, 0))
      resolveLastPendingCommand(deps.connections, response)
    ...
    it('sendPackageCommand - PackageList (no package)', async () => {
      const result = await testWrapper(() =>
        dispatcher.sendPackageCommand('vm-1', 'PackageList'),
      )
      expect(result.success).toBe(true)
      const [, msg] = (deps.sendMessage as jest.Mock).mock.calls[0]
      expect(msg.command_type).toEqual({ action: 'PackageList' })
    })

    it('sendPackageCommand - PackageInstall with package name', async () => {
      const result = await testWrapper(() =>
        dispatcher.sendPackageCommand('vm-1', 'PackageInstall', 'nginx'),
      )
      expect(result.success).toBe(true)
      const [, msg] = (deps.sendMessage as jest.Mock).mock.calls[0]
    ...
    it('sendPackageCommand - PackageSearch uses query param', async () => {
      const result = await testWrapper(() =>
        dispatcher.sendPackageCommand('vm-1', 'PackageSearch', 'python'),
      )
      expect(result.success).toBe(true)
      const [, msg] = (deps.sendMessage as jest.Mock).mock.calls[0]
    ...
    it('sendProcessCommand - ProcessList', async () => {
      const result = await testWrapper(() =>
        dispatcher.sendProcessCommand('vm-1', 'ProcessList', { limit: 50 }),
      )
      expect(result.success).toBe(true)
      const [, msg] = (deps.sendMessage as jest.Mock).mock.calls[0]
    ...
    it('getUserList sends UserList action', async () => {
      const result = await testWrapper(() =>
        dispatcher.getUserList('vm-1'),
      )
      expect(result.success).toBe(true)
      const [, msg] = (deps.sendMessage as jest.Mock).mock.calls[0]
    ...
    it('sendMaintenancePowerShellScript sends with correct params', async () => {
      const result = await testWrapper(() =>
        dispatcher.sendMaintenancePowerShellScript('vm-1', 'Get-Service', {
          scriptType: 'inline',
          timeoutSeconds: 120,
          runAsAdmin: true,
        }),
    ...
    it('sendCleanTemporaryFiles sends with targets', async () => {
      const result = await testWrapper(() =>
        dispatcher.sendCleanTemporaryFiles('vm-1', ['/tmp', '/var/tmp']),
      )
      expect(result.success).toBe(true)
      const [, msg] = (deps.sendMessage as jest.Mock).mock.calls[0]
