...
const FirewallPolicyService_1 = require("../../../../app/services/firewall/FirewallPolicyService");
describe('FirewallPolicyService', () => {
    ...
    beforeEach(() => {
        ...
        service = new FirewallPolicyService_1.FirewallPolicyService(mockPrisma, mockRuleService);
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    describe('generateDefaultRules', () => {
        describe('BLOCK_ALL policy', () => {
            it('should generate rules for allow_internet preset', () => {
                const rules = service.generateDefaultRules(client_1.FirewallPolicy.BLOCK_ALL, 'allow_internet');
            ...
            it('should generate rules for allow_outbound preset', () => {
                const rules = service.generateDefaultRules(client_1.FirewallPolicy.BLOCK_ALL, 'allow_outbound');
                expect(rules).toHaveLength(2); // 1 established + 1 allow all outbound
                expect(rules[1].name).toBe('Allow All Outbound (System)');
                expect(rules[1].action).toBe(client_1.RuleAction.ACCEPT);
                expect(rules[1].direction).toBe(client_1.RuleDirection.OUT);
                expect(rules[1].protocol).toBe('all');
            });
            it('should generate no rules for block_all preset', () => {
                const rules = service.generateDefaultRules(client_1.FirewallPolicy.BLOCK_ALL, 'block_all');
            ...
            it('should default to allow_outbound for unknown BLOCK_ALL config', () => {
                const rules = service.generateDefaultRules(client_1.FirewallPolicy.BLOCK_ALL, 'unknown_config');
        ...
        describe('ALLOW_ALL policy', () => {
            it('should generate blocking rules for block_ssh preset', () => {
                const rules = service.generateDefaultRules(client_1.FirewallPolicy.ALLOW_ALL, 'block_ssh');
                expect(rules).toHaveLength(3); // 1 established + 2 blocking rules
                const blockingRules = rules.filter(r => r.action === client_1.RuleAction.DROP);
            ...
            it('should generate blocking rules for block_smb preset', () => {
                const rules = service.generateDefaultRules(client_1.FirewallPolicy.ALLOW_ALL, 'block_smb');
            ...
            it('should generate blocking rules for block_databases preset', () => {
                const rules = service.generateDefaultRules(client_1.FirewallPolicy.ALLOW_ALL, 'block_databases');
                expect(rules).toHaveLength(6); // 1 established + 5 database ports
                const blockingRules = rules.filter(r => r.action === client_1.RuleAction.DROP);
                expect(blockingRules).toHaveLength(5);
                expect(blockingRules.map(r => r.name)).toEqual(expect.arrayContaining([
                    'Block MySQL (System)',
            ...
            it('should generate no blocking rules for none preset', () => {
                const rules = service.generateDefaultRules(client_1.FirewallPolicy.ALLOW_ALL, 'none');
                expect(rules).toHaveLength(1); // Only the established connection rule
            });
            it('should handle unknown ALLOW_ALL configs gracefully', () => {
                const rules = service.generateDefaultRules(client_1.FirewallPolicy.ALLOW_ALL, 'unknown_config');
                expect(rules).toHaveLength(1); // Only the established connection rule
            });
        });
        it('should always include established connections rule', () => {
            ...
            expect(rules[0].name).toBe('Allow Established Connections (System)');
            expect(rules[0].priority).toBe(50);
            expect((_a = rules[0].connectionState) === null || _a === void 0 ? void 0 : _a.states).toEqual(['ESTABLISHED', 'RELATED']);
        });
    });
    describe('applyPolicyToRuleSet', () => {
        it('should delete existing system-generated rules and create new ones', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            yield service.applyPolicyToRuleSet(ruleSetId, client_1.FirewallPolicy.BLOCK_ALL, 'allow_internet');
        ...
        it('should work with empty rule set', () => __awaiter(void 0, void 0, void 0, function* () {
            const ruleSetId = 'ruleset-456';
            jest.spyOn(mockRuleService, 'deleteSystemGeneratedRules').mockResolvedValue(0);
            jest.spyOn(mockRuleService, 'createRule').mockResolvedValue({ id: 'new-rule' });
            yield service.applyPolicyToRuleSet(ruleSetId, client_1.FirewallPolicy.ALLOW_ALL, 'block_ssh');
            expect(mockRuleService.deleteSystemGeneratedRules).toHaveBeenCalledWith(ruleSetId);
        }));
    });
    describe('ensureAndApplyDepartmentPolicy', () => {
        it('should create rule set if one does not exist', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const result = yield service.ensureAndApplyDepartmentPolicy('dept-123', client_1.FirewallPolicy.BLOCK_ALL, 'allow_internet');
            expect(result).toBe('ruleset-new');
            expect(mockRuleService.createRuleSet).toHaveBeenCalledWith(client_1.RuleSetType.DEPARTMENT, 'dept-123', 'Engineering Firewall', expect.stringContaining('ibay-dept-'));
            expect(mockPrisma.department.update).toHaveBeenCalledWith({
                where: { id: 'dept-123' },
                data: { firewallRuleSetId: 'ruleset-new' }
            });
        }));
        it('should use existing rule set if one exists', () => __awaiter(void 0, void 0, void 0, function* () {
            const mockDepartment = {
                id: 'dept-123',
                name: 'Engineering',
            ...
            mockPrisma.department.findUnique.mockResolvedValue(mockDepartment);
            yield service.ensureAndApplyDepartmentPolicy('dept-123', client_1.FirewallPolicy.BLOCK_ALL, 'allow_internet');
        ...
        it('should throw error if department not found', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            yield expect(service.ensureAndApplyDepartmentPolicy('nonexistent', client_1.FirewallPolicy.BLOCK_ALL, 'allow_internet')).rejects.toThrow('Department nonexistent not found');
        }));
    });
    describe('getPolicyPresetDescription', () => {
        describe('BLOCK_ALL policies', () => {
            it('should return description for allow_internet', () => {
                const description = service.getPolicyPresetDescription(client_1.FirewallPolicy.BLOCK_ALL, 'allow_internet');
            ...
            it('should return description for allow_outbound', () => {
                const description = service.getPolicyPresetDescription(client_1.FirewallPolicy.BLOCK_ALL, 'allow_outbound');
            ...
            it('should return description for block_all', () => {
                const description = service.getPolicyPresetDescription(client_1.FirewallPolicy.BLOCK_ALL, 'block_all');
            ...
            it('should return unknown message for unknown config', () => {
                const description = service.getPolicyPresetDescription(client_1.FirewallPolicy.BLOCK_ALL, 'unknown');
                expect(description).toBe('Unknown configuration');
            });
        });
        describe('ALLOW_ALL policies', () => {
            it('should return description for block_ssh', () => {
                const description = service.getPolicyPresetDescription(client_1.FirewallPolicy.ALLOW_ALL, 'block_ssh');
                expect(description).toContain('Allows all traffic except SSH');
            });
            it('should return description for block_smb', () => {
                const description = service.getPolicyPresetDescription(client_1.FirewallPolicy.ALLOW_ALL, 'block_smb');
                expect(description).toContain('SMB file sharing');
            });
            it('should return description for block_databases', () => {
                const description = service.getPolicyPresetDescription(client_1.FirewallPolicy.ALLOW_ALL, 'block_databases');
            ...
            it('should return description for none', () => {
                const description = service.getPolicyPresetDescription(client_1.FirewallPolicy.ALLOW_ALL, 'none');
                expect(description).toContain('Allows all traffic');
            });
            it('should return unknown message for unknown config', () => {
                const description = service.getPolicyPresetDescription(client_1.FirewallPolicy.ALLOW_ALL, 'unknown');
                expect(description).toBe('Unknown configuration');
            });
        });
    });
