import { PrismaClient } from '@prisma/client'
import forDeleteDisk from '../app/graphql/resolvers/mutations/deleteDisk.js'
import { describe, expect, beforeEach, afterEach, afterAll, test } from '@jest/globals'
const prisma = new PrismaClient()

describe('forDeleteDisk function', () => {
  let mockInput
  beforeEach(() => {
    mockInput = {
      input: {
        token: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6ImUxMDE5YjJhLTgwNGUtNDI4MC1hNjVkLTc5NzkxZjg5MjIwNyIsImVNYWlsIjoiZml6emFmYXRpbWE2NDJAZ21haWwuY29tIiwidXNlclR5cGUiOiJ1c2VyIiwiaWF0IjoxNjgwODQwNTczLCJleHAiOjE3NjcyNDA1NzN9.5cfptEbE_QN6EazYxJ0i1mOlgJsYEwZRpXnJt4Qfm_o',
        id: '1cc95ba7-95bf-441d-85a9-f5f1049a5862'
      }
    }
    jest.mock('../app/services/isAuthForUser.js', () => {
      return jest.fn().mockImplementation(() => ({
        id: 'e1019b2a-804e-4280-a65d-79791f892207'
      }))
    })
  })
  afterEach(() => {
    jest.clearAllMocks()
  })
  afterAll(async () => {
    await prisma.$disconnect()
  })
  test('should delete a disk in database', async () => {
    const result = await forDeleteDisk.Mutation.deleteDisk(null, mockInput)
    expect(result).toEqual('Deleted Disk')

    const deletedDisk = await prisma.disk.findUnique({ where: { id: mockInput.input.id } })
    expect(deletedDisk).toBeNull()
  })

  test('should throw an error if the function fails', async () => {
    jest.spyOn(prisma.disk, 'delete').mockRejectedValue(new Error('mockError'))
    try {
      await forDeleteDisk.Mutation.deleteDisk(null, mockInput)
    } catch (error) {
      expect(error).toBeInstanceOf(Error)
      expect(error.message).toBe('Failed')
    }
  })
})
